...
def main(args):
    ...
    model = Qwen3ASRModel.from_pretrained(
        args.model_id,
        dtype=dtype,
        device_map="cpu" if device_str.startswith("cuda") else "cpu",
    ...
    def benchmark(batch):
        ...
        # Compute total audio duration (in seconds)
        total_audio_sec = 0.0
        for a in audios:
            total_audio_sec += len(a["array"]) / a["sampling_rate"]

        # Real-Time Factor (RTFx)
        rtfx = total_audio_sec / runtime if runtime > 0 else float("inf")
        print("RTFXXXXXXX")
        print(rtfx)
